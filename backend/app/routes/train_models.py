...
def get_base_path():
    ...
    """
    if os.path.exists("/app"):  # estamos dentro do Docker
        return "/app"
    else:
        return os.path.abspath(os.path.join(os.getcwd(), "pipeline_train", "backend", "app"))
...
def get_path(*parts):
    ...
    """
    path = os.path.join(get_base_path(), *parts)
    return path.replace("\\", "/")
