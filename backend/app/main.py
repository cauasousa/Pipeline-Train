...
# New root-level endpoint to list available models (so GET /models works)
@app.get("/models", tags=["models"])
async def list_models_root():
    base = os.path.join(os.path.dirname(__file__), "storage", "models_yolo")
    print("Caminho base:", base)
    if not os.path.exists(base):
        print("⚠️ Diretório não existe")
    models = [f for f in sorted(os.listdir(base)) if os.path.isfile(os.path.join(base, f))] if os.path.exists(base) else []
    print("MODELS encontrados:", models)
